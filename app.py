...
# Reduce thread/parallelism overhead — helps keep peak memory a bit lower
# on constrained containers (e.g. Streamlit Community Cloud's free tier).
...
# ----------------------------------------------------------------------------
# Model loading — split into two lazily-cached loaders so the segmentation
# model is only pulled into memory when a tumor is actually detected. This
# keeps peak memory lower on constrained hosts (e.g. Streamlit Community
# Cloud's free 1GB limit) for "no tumor" cases.
# ----------------------------------------------------------------------------
@st.cache_resource
def load_classifier():
    ensure_model_downloaded(CLS_MODEL_PATH, CLS_MODEL_DRIVE_ID)
    # compile=False: we only need this model for inference, not training,
    ...
    return tf.keras.models.load_model(CLS_MODEL_PATH, compile=False)
